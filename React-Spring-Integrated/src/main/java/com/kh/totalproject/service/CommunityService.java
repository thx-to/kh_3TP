...
public class CommunityService {
    ...
    public Boolean createPost(BoardRequest boardRequest, String boardType) {
        try {
            ...
            boolean isSavedToMySQL = saveBoardEntity(boardEntity);
            // SQL 에 저장이 됐다면, 엘라스틱서치에도 똑같이 필요한 필드 저장
            if (isSavedToMySQL) {
                saveToElasticsearch(boardEntity);
            } else {
                // MySQL 저장 후 Elasticsearch 에 저장 실패시
                System.err.println("게시글을 엘라스틱 서치에 전달 할 수 없습니다.");
            }
            return isSavedToMySQL;
    ...
    private void saveToElasticsearch(Board boardEntity) {
        try {
            ...
            log.info("Elasticsearch 저장 시도: id={}, title={}, content={}, boardType={}, result={}",
                    boardEntity.getId(), boardEntity.getTitle(),
                    boardEntity.getContent(), boardEntity.getBoardType(), response.result());
        } catch (Exception e) {
            log.error("Elasticsearch 저장 실패: {}", boardEntity.getId(), e);
        }
    }
    ...
    private void deleteFromElasticsearch(Long boardId) {
        try {
            ...
            log.error("Elasticsearch 에서 삭제중 에러 발생{}", boardId, e);
        } catch (IOException e) {
    ...
    private void updateElasticsearchForSave(Board boardEntity) {
        try {
            ...
            log.info("Elasticsearch 문서 저장 성공: {}", boardEntity.getId());
        } catch (Exception e) {
            log.error("Elasticsearch 저장 실패: {}", boardEntity.getId(), e);
        }
    }
