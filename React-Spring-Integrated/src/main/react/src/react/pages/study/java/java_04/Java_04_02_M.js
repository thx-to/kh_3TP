    ...
    <Wrap>
      ...
      <Container>
        ...
        <ClassContentsText>
          ...
          를 가집니다.
          <br />
          메소드와 필드는 객체지향 프로그래밍의 핵심 요소로서 클래스 내에서
          정의되며, 객체의 동작과 상태를 나타내는 역할을 합니다.
          <br />
          메소드는 함수와 거의 동일 합니다. (자바에서는 함수가 단독으로 존재할
          수 없는 클래스 기반의 언어라는 차이점이 있습니다.)
        </ClassContentsText>
        <br />
        ...
        <ClassContentsText>
          ...
          <br />
          메소드는 특정 작을 수행하거나 계산을 수행하며, 필요한 경우 메소드에
          매개변수를 전달 할 수 있습니다.
          <br />
        ...
        <ClassContentsText>
          함수란 <b>‘하나의 기능을 수행하는 일련의 코드’</b>를 말합니다.
          <ClassContentsImage
            style={{
              width: "100%",
              height: "200px",
        ...
        <ClassContentsText>
          ...
          <br />
          프로그램의 문제점 발견이나 기능을 추가하거나 변경하기 쉽습니다.
          <br />
          메소드를 작성할 때는 가급적 하나의 메소드가 하나의 기능만을 수행하도록
          작성하는 것이 좋습니다.
        </ClassContentsText>
        <br />
        ...
        <ClassContentsText>
          ...
          - <b>접근제한자</b> : 해당 메소드에 접근 할 수 있는 범위를 명시합니다.
          <br />- <b>반환 타입</b> : 메소드가 작업을 마치고 반환하는 데이터의
          타입을 명시합니다.
          <br />- <b>메소드 이름</b> : 메소드를 호출하기 위한 이름입니다.
          <br />- <b>매개변수 목록</b>(parameters) : 메소드 호출 시 전달되는
          인수의 값을 저장할 변수들을 명시합니다.
          <br />- 구현부 : 메소드의 고유 기능을 수행하는 명령문의 집합 입니다.
        </ClassContentsText>
        <br />
        <ClassContentsTitle2>메소드의 선언</ClassContentsTitle2>
        <ClassContentsTitle3>메소드 이름</ClassContentsTitle3>
        <ClassContentsText>
          메소드의 이름은 자바 식별자 규칙에 맞게 작성하면 되는데 아래의 규칙을
          만족해야 합니다.
          <br />
          - 숫자로 시작하면 안됨
          <br />
        ...
        <ClassContentsText>
          ...
          <br />
          <b>
            메소드는 리턴값이 있을 수도 있고 없을 수도 있으나 리턴 타입은
            선언부에 명시
          </b>
          되어야 합니다.
        </ClassContentsText>
        ...
        <ClassContentsText>
          - 매개변수(인자)는 메소드의 선언문에서 작은 괄호 내의 변수를 의미
          <br />- 인수는 메소드의 호출부에서 전달하는 값을 의미
          <Java_04_02_Code01 />
        </ClassContentsText>
        <br />
        <ClassContentsTitle3>입력값도 리턴값도 없는 메소드</ClassContentsTitle3>
        <Java_04_02_Code02 />
        <br />
        <ClassContentsTitle3>매개 변수의 수를 모를 경우</ClassContentsTitle3>
        <ClassContentsText>
          메소드의 배개 변수는 개수가 이미 정해져 있는것이 일반적이지만, 경우에
          따라서는 메소드를 선언할 때 변수의 개수를 알 수 없는 경우가 있습니다.
          이럴 경우 … 을 사용하면 됩니다.
          <Java_04_02_Code03 />
        </ClassContentsText>
        <br />
        ...
        <ClassContentsText>
          ...
          <br />
          <b>
            클래스 내부에서 다른 메소드 호출 시는 단순히 메소드 이름으로
            호출하면 되고, 클래스 외부에서 호출 할 때는 객체를 생성한 뒤 참조
            변수를 이용해서 메소드를 호출
          </b>
          해야 합니다. (뒤에 나오는 영화표 예매 예제를 통해서 확인 가능)
        </ClassContentsText>
        <br />
        <ClassContentsTitle2>재귀 호출(recursive call)</ClassContentsTitle2>
        <ClassContentsText>
          재귀호출이란{" "}
          <b>메소드 내부에서 해당 메소드가 또 다시 호출되는 것을 의미</b>
          합니다.
          <br />
          재귀호출은 자기가 자신을 계속해서 호출하므로, 끝없이 반복 됩니다.
          (반드시 탈출문 필요)
          <br />
          재귀호출은 잘못 사용하는 경우 무한 루프에 빠지는 등 문제가 발생 할
          가능성이 높고 이해하기 어려운 문법이지만 알고리즘과 자료구조에서 매우
          중요하게 사용되고 있어 중요한 개념중에 하나 입니다.
          <br />
          또한, 재귀 호출을 사용하면 복잡한 문제도 매우 간단하게 논리적으로
          접근하여 표현 할 수 있습니다.
        </ClassContentsText>
        <ClassContentsTitle3>
          n까지의 합을 더하는 재귀호출 함수
        ...
        <ClassContentsText>
          <b>자바프로그래밍에서 ‘필드’는 클래스 내부에 선언된 변수를 의미</b>
          합니다. 필드는 클래스의 객체들이 공유하는 데이터를 저장하며, 객체의
          상태를 나타내는 역할을 합니다. 필드는 클래스의 중요한 구성 요소 중
          하나로, 클래스의 특징과 행동을 정의하는 데 사용됩니다.
          <br />
          <br />
          1. <b>데이터 저장</b>
          <br />
          <ClassContentsTextTab>
            필드는 <b>데이터를 저장</b>하는 역할을 합니다. 클래스가 특정 속성을
            가지거나 특정 값을 유지해야 할 때 필드를 사용하여 그 정보를
            저장합니다. 예를 들어, 자동차 클래스의 필드로는 브랜드, 모델, 연식
            등이 있을 수 있습니다.
          </ClassContentsTextTab>
          2. <b>인스턴스 변수</b>
          <br />
          <ClassContentsTextTab>
            필드는 클래스의 객체(인스턴스)마다 각각의 복사본을 가지게 됩니다.
            따라서 각 객체마다 다른 값을 가질 수 있습니다. 이를 통해 객체마다
            고유한 상태를 유지할 수 있습니다.
          </ClassContentsTextTab>
          3. <b>접근 제어자</b>
          <br />
          <ClassContentsTextTab>
            ...
            <ClassContentsCode>public</ClassContentsCode> 등)를 사용하여 접근
            범위를 제한할 수 있습니다. 이를 통해 필드에 직접 접근하는 것을
            제어하거나 제한할 수 있습니다.
          </ClassContentsTextTab>
          4. <b>데이터 타입</b>
          <br />
          <ClassContentsTextTab>
            필드는 데이터 타입을 가집니다. 이는 필드가 어떤 종류의 데이터를
            저장하는지를 나타냅니다. 정수, 실수, 문자열 등 다양한 데이터 타입을
            필드로 사용할 수 있습니다.
          </ClassContentsTextTab>
          5. <b>초기값 설정</b>
          <br />
          <ClassContentsTextTab>
            필드는 선언할 때 초기값을 설정할 수 있습니다. 이 초기값은 필드가
            객체 생성 시에 할당됩니다. 만약 초기값이 설정되지 않으면 기본값(0,
            false, null 등)이 할당됩니다.
          </ClassContentsTextTab>
          6. <b>클래스 변수</b>
          <br />
          <ClassContentsTextTab>
            필드는 <ClassContentsCode>static</ClassContentsCode> 키워드로
            선언하여 클래스 변수로 만들 수도 있습니다. 클래스 변수는 클래스의
            모든 객체가 공유하는 하나의 데이터를 나타냅니다. 예를 들어, 모든
            객체가 공통적으로 사용하는 상수를 클래스 변수로 선언할 수 있습니다.
          </ClassContentsTextTab>
          7. <b>상태 정보 저장</b>
          <br />
          <ClassContentsTextTab>
            필드는 객체의 상태 정보를 저장하는 데 사용됩니다. 객체의 특성이나
            속성을 나타내며, 객체의 상태가 변화할 때 필드의 값도 함께 변화할 수
            있습니다.
          </ClassContentsTextTab>
          8. <b>메서드와 함께 사용</b>
          <br />
          <ClassContentsTextTab>
            필드의 값은 클래스 내부의 메서드에서 사용되어 객체의 동작에 영향을
            줄 수 있습니다. 필드의 값을 변경하거나, 필드의 값을 기반으로 계산을
            수행하는 등의 작업이 가능합니다.
          </ClassContentsTextTab>
          9. <b>Getter와 Setter 메서드</b>
          <br />
          <ClassContentsTextTab>
            필드의 값을 가져오기 위한 Getter 메서드와 값을 설정하기 위한 Setter
            메서드를 제공하여 필드에 접근하고 수정할 수 있습니다.
          </ClassContentsTextTab>
          <br />
          <Java_04_02_Code05 />
          <br />
          <ClassTableBox>
            <ClassTable style={{ textAlign: "center" }}>
              ...
              <ClassTableTr>
                ...
                <ClassTableTd>클래스가 메모리에 올라갈 때</ClassTableTd>
          ...
          <br />
          클래스 변수는 인스턴스를 생성하지 않고도 바로 사용 할 수 있습니다.
          <br />
          <b>
            클래스 변수와 인스턴스 변수는 초기화 하지 않아도 변수의 타입에 맞게
            자동으로 초기화
          </b>
          됩니다.
          <br />
        ...
        <ClassContentsText>
          <b>
            ...
            초기화할 수 있는 생성자라는 메소드를 제공
          </b>
          합니다.
          <br />
          <b>new 연산자로 클래스로 부터 객체를 생성 할 때 자동으로 호출</b>
          됩니다.
          <br />- 생성자와 클래스의 이름은 같아야 합니다.
          <br />- 생성자는 반환값이 없지만, void로 반환타입을 선언하지 않습니다.
          <br />- 하나의 클래스가 여러 개의 생성자를 가질 수 있습니다. (생성자
          오버로딩)
        </ClassContentsText>
        ...
        <ClassContentsText>
          <b>클래스로 부터 객체가 생성될 때 필드는 기본 초기값으로 자동 설정</b>
          됩니다.
          <br />
          객체의 생성 시점에 외부에서 제공되는 다양한 값들로 초기화 되어야
          한다면 <b>생성자에서 초기화</b>해야 합니다.
        </ClassContentsText>
        {/* </ClassContentsContainer>
          </EachClass> */}
        <ArrowContainer>
