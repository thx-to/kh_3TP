    ...
    <Wrap>
      ...
      <TopBoxWide2>
        <TopBox2>
          ...
          <TopBoxText2 onClick={() => handleRefresh()}>07. 상속</TopBoxText2>
        </TopBox2>
      </TopBoxWide2>
      <Container>
        {/* <EachClass>
        ...
        <ClassContentsText>
          -{" "}
          <b>
            상속(Inheritance)은 자식 클래스가 부모 클래스의 기능을 물려받아
            사용할 수 있게 하는 것
          </b>
          입니다.
          <br />- 상속을 통해 코드 재사용성을 높이고, 새로운 클래스의 구현을
          쉽게 할 수 있습니다.
          <br />- 상속을 구현하려면{" "}
          <ClassContentsCode>extends</ClassContentsCode> 키워드를 사용합니다.
        </ClassContentsText>
        <ClassContentsTitle3>상속의 주요 특징</ClassContentsTitle3>
        <ClassContentsText>
          - <b>재사용성</b> : 기존 클래스를 재사용하여 새로운 클래스를 작성할 수
          있습니다.
          <br />- <b>확장성</b> : 기존 코드를 수정하지 않고 새로운 기능을
          추가하거나 기존 기능을 확장할 수 있습니다.
          <br />- <b>다형성</b> : 상속을 통해 메소드 오버라이딩(Overriding)을
          구현하여 다형성을 실현할 수 있습니다. 오버라이딩은 자식 클래스에서
          부모 클래스의 메소드를 재정의하는 것을 말합니다.
          <br />
        </ClassContentsText>
        ...
        <ClassContentsText>
          ...
          <br />- 클래스 간의 계층적 관계를 구성하여 <b>다형성</b>을 쉽게 구현할
          수 있습니다.
        </ClassContentsText>
        <ClassContentsTitle3>
          부모 클래스와 자식 클래스의 관계
        ...
        <ClassContentsText>
          자바는 <b>다단계 상속</b>과 <b>계층적 상속</b>을 지원합니다. 다만{" "}
          <b>다중 상속</b>은 지원하지 않습니다. 이는 다중 상속으로 인한 모호성
          문제를 피하기 위함입니다. 대신
          <b>인터페이스</b>를 통해 다중 상속과 유사한 기능을 제공합니다.
          <ClassContentsImage
            style={{
              width: "100%",
              height: "200px",
        ...
        <ClassContentsText>
          클래스 상속을 위해 <ClassContentsCode>extends</ClassContentsCode>{" "}
          키워드를 사용합니다. 아래의 예시에서는{" "}
          <ClassContentsCode>Dog</ClassContentsCode> 클래스가{" "}
          <ClassContentsCode>Animal</ClassContentsCode>
          클래스를 상속받아 <ClassContentsCode>Animal</ClassContentsCode>{" "}
          클래스의 필드와 메소드를 그대로 사용합니다.
          <Java_04_07_Code01 />
        </ClassContentsText>
        <ClassContentsTitle3>
        ...
        <Java_04_07_Code03 />
        <br />
        <ClassContentsTitle2>final 클래스와 final 매소드</ClassContentsTitle2>
        <ClassContentsText>
          final 키워드는 클래스, 필드, 매소드 선언 시에 사용 할 수 있다. final
          키워드는 해당 선언이 최종 상태이고 수정 할 수 없음을 나타낸다.
        </ClassContentsText>
        <ClassContentsTitle3>상속 할 수 없는 final 클래스</ClassContentsTitle3>
        <ClassContentsText>
          상속 할 수 없는 대표적인 클래스는 String클래스 이다. (보안 문제)
        </ClassContentsText>
        <ClassContentsTitle3>
        ...
        <ClassContentsText>
          <ClassContentsCode>static final</ClassContentsCode>은 자바에서 상수를
          정의하는 데 사용되는 키워드입니다.{" "}
          <ClassContentsCode>static final</ClassContentsCode>을 사용하여 정의된
          변수는 클래스 수준에서 한 번 초기화되고 이후에 값이 변경되지 않는
          상수가 됩니다. 이렇게 정의된 상수는 클래스 내부에서 사용되며, 객체의
          생성 여부와 관계없이 항상 동일한 값을 갖습니다. 아래는{" "}
        ...
        <ClassContentsText>
          자바는 다중 상속을 지원하지 않는 프로그래밍 언어입니다. 이러한 선택은
          몇 가지 이유로 인해 이루어졌습니다.
          <br />
          1. <b>다이아몬드 문제(Diamond Problem)</b>
          <br />
          <ClassContentsTextTab>
            다중 상속이 허용되는 언어에서는 같은 메서드나 속성을 둘 이상의 부모
            클래스로부터 상속받을 수 있습니다. 이 경우에 상속 계층이 다이아몬드
            형태로 형성될 수 있고, 이로 인해 충돌이 발생하는 다이아몬드 문제가
            발생할 수 있습니다. 즉, 동일한 메서드나 속성을 어떤 부모 클래스의
            것으로 사용해야 할지 결정하기 어려워집니다.
          </ClassContentsTextTab>
          2. <b>복잡성 증가</b>
          <br />
          <ClassContentsTextTab>
            다중 상속을 지원하는 언어에서는 클래스 계층 구조가 복잡해질 수
            있습니다. 코드를 이해하고 유지보수하기 어려워질 수 있습니다. 또한
            상속 체인의 변경이 한 부분에서 다른 부분으로 예상치 못한 영향을 미칠
            수 있습니다.
          </ClassContentsTextTab>
          3. <b>클래스 간 명확한 관계 부재</b>
          <br />
          <ClassContentsTextTab>
            다중 상속이 허용되면 클래스 간의 명확한 관계를 파악하기 어려워질 수
            있습니다. 어떤 클래스의 메서드나 속성이 어떤 부모 클래스에서
            상속되었는지 추적하기가 어려울 수 있습니다.
          </ClassContentsTextTab>
          4. <b>자바의 설계 철학</b>
          <br />
          <ClassContentsTextTab>
            자바는 단순하고 명확한 객체 지향 프로그래밍을 지향하는 언어입니다.
            다중 상속을 허용함으로써 발생하는 복잡성과 충돌을 피하기 위해 클래스
            단위의 단순한 상속 구조를 채택하였습니다.
          </ClassContentsTextTab>
          <br />
          자바는 이러한 이유로 인해 다중 상속을 지원하지 않습니다. 그 대신
          인터페이스를 통한 다중 상속과 유사한 기능을 제공하여 클래스 간의
          관계와 코드의 구조를 보다 명확하게 유지할 수 있도록 도와줍니다.
        </ClassContentsText>
        <ClassContentsTitle3>
        ...
        <br />
        <ClassContentsTitle2>객체 타입 확인(instanceof)</ClassContentsTitle2>
        <ClassContentsText>
          <ClassContentsCode>
            boolean result = 좌향(객체) instanceof 우향(타입)
          </ClassContentsCode>
          <br />
          <ClassContentsCode>instanceof</ClassContentsCode> 연산자는 객체가 특정
          클래스의 인스턴스인지를 확인하기 위해 사용됩니다. 이 연산자는 두 개의
          피연산자를 받아서 왼쪽 피연산자가 오른쪽 피연산자의 인스턴스인지를
          검사하며, 결과는 불리언 값(<ClassContentsCode>true</ClassContentsCode>{" "}
          또는
          <ClassContentsCode>false</ClassContentsCode>)으로 반환됩니다.
          <br />
          <b>주로 상속 관계에서 부모객체인지 자식 객체인지 확인 하는데 사용</b>
          됩니다.
          <Java_04_07_Code09 />
        </ClassContentsText>
        {/* </ClassContentsContainer>
