    ...
    <Wrap>
      ...
      <TopBoxWide2>
        <TopBox2>
          ...
          <TopBoxText2 onClick={() => handleRefresh()}>03. List</TopBoxText2>
        </TopBox2>
      </TopBoxWide2>
      <Container>
        {/* <EachClass>
        ...
        <ClassContentsText>
          리스트는 배열과 비슷한 자바의 자료형으로 배열보다 편리한 기능을 많이
          가지고 있습니다.
          <br />
          리스트와 배열의 가장 큰 차이점은 크기가 정해져 있지 않고 동적으로
          변한다는 점 입니다.
          <br />
        ...
        <ClassContentsTitle3>자바4 이전 표시 방법</ClassContentsTitle3>
        <ClassContentsText>
          리스트 생성 시 컴파일 타임에 객체의 타입을 확인하지 않습니다.
          <Java_06_03_Code01 />
        </ClassContentsText>
        <ClassContentsTitle3>자바5 이후 표시 방법</ClassContentsTitle3>
        <Java_06_03_Code02 />
        <ClassContentsTitle3>List 컬렉션 클래스의 종류</ClassContentsTitle3>
        <ClassContentsText>
        ...
        <ClassContentsText>
          개체의 배열을 구현한 클래스이며 컬렉션 인터페이스와 그 하위의 List
          인터페이스로 구현 합니다.
          <br />
          객체 순서를 기반으로 순차적으로 자료를 관리하는 프로그램을 구현 할 때
          사용 합니다.
          <Java_06_03_Code03 />
          ...
           처럼 객체를 포함하는 자료형도 어떤 객체를 포함하는지에 대해서
          명확하게 표현할 것을 권고하고 있다.
        </ClassContentsText>
        ...
        <ClassContentsText>
          리스트 안에 해당 항목이 있는지를 판별하여 그 결과를 boolean으로
          리턴한다.
          <Java_06_03_Code07 />
        </ClassContentsText>
        <ClassContentsTitle3>remove</ClassContentsTitle3>
        <ClassContentsText>
          remove(객체) : 리스트에서 객체에 해당되는 항목을 삭제하고 삭제한
          결과(true, false)를 리턴
          <Java_06_03_Code08 />
        ...
        <ClassContentsText>
          - 오름차순 정렬 :{" "}
          <ClassContentsCode>Comparator.naturalOrder()</ClassContentsCode>
          <br />- 내림차순 정렬 :{" "}
          <ClassContentsCode>Comparator.reverseOrder()</ClassContentsCode>
          <br />- Comparator의 compare() 메서드롤 오버라이딩해서 정렬 방법을
          만들 수 있습니다.
          <Java_06_03_Code13 />
        </ClassContentsText>
        <ClassContentsTitle3>클래스를 ArrayList로 구성</ClassContentsTitle3>
        <Java_06_03_Code14 />
        <ClassContentsTitle3>
        ...
        <ClassContentsText>
          ...
          <br />
          LinkedList 클래스는 ArrayList 클래스가 배열을 이용하여 요소를
          저장함으로써 발생하는 단점을 극복하기 위해 고안되었습니다.
          <br />
          JDK 1.2부터 제공된 LinkedList 클래스는 내부적으로 연결 리스트(linked
          list)를 이용하여 요소를 저장합니다.
          <br />
          배열은 저장된 요소가 순차적으로 저장됩니다.
          <br />
          하지만 연결 리스트는 저장된 요소가 비순차적으로 분포되며, 이러한
          요소들 사이를 링크(link)로 연결하여 구성합니다.
          <br />
        ...
        <ClassContentsText>
          Vector는 ArrayList와 동일한 내부 구조를 가지고 있습니다. (사용 방법
          동일)
          <br />
          <ClassContentsCode>
          ...
          ArrayList와 다른점은{" "}
          <b>
            동기화된(synchronized) 메소드로 구성되어 있어 멀티 쓰레드 환경에서
            안전
          </b>
        ...
        <ClassContentsText>
          ...
          <br />
          <ClassContentsTextTab>
            - BigDecimal은 자바에서 고정소수점 및 부동 소수점 연산의 정확성을
            요구할 때 사용되는 클래스
            <br />- 금융 계산이나 높은 정밀도가 필요한 경우 double이나 float보다
            더 안정하게 사용
          </ClassContentsTextTab>
        </ClassContentsText>
        <ClassContentsTitle3>
        ...
        <Java_06_03_Code19 />
        <ClassContentsText>
          - <ClassContentsCode>RoundingMode.UP</ClassContentsCode> : 무조건 올림
          <br />- <ClassContentsCode>RoundingMode.DOWN</ClassContentsCode> :
          무조건 내림
          <br />- <ClassContentsCode>RoundingMode.HALF_UP</ClassContentsCode> :
          소수점 중간값에서 올림
        ...
        <ClassContentsText>
          BigDecimal에서 스케일(scale)은 소수점 이하 자릿수를 나타내며,
          정밀도(precision)는 전체 자릿수를 의미
          <Java_06_03_Code20 />
        </ClassContentsText>
        <br />
        <ClassContentsTitle3>
        ...
        <ClassContentsText>
          ...
          <br />- <ClassContentsCode>Product.java</ClassContentsCode> 파일을
          프로젝트 폴더에 추가하고{" "}
          <ClassContentsCode>Product</ClassContentsCode> 클래스를 정의하세요.
          <br />- 모든 <ClassContentsCode>Product</ClassContentsCode> 객체가
          가질 멤버 변수를 정의하세요.
          <ClassContentsTextTab>
            · <ClassContentsCode>name</ClassContentsCode> : 제품의 이름을
            저장하는 변수 (예시: "Apples")
            <br />· <ClassContentsCode>price</ClassContentsCode> : 제품 가격을{" "}
            <ClassContentsCode>BigDecimal</ClassContentsCode>로 저장하는 변수 (
            <ClassContentsCode>float</ClassContentsCode>나{" "}
            <ClassContentsCode>double</ClassContentsCode>을 사용하면 과제에서
            0점을 받습니다)
            <br />· <b>중요</b> : 여기서 요구된 멤버 변수 외에 추가적인 멤버
            변수를 정의하지 마세요.
          </ClassContentsTextTab>
          - 두 개의 입력을 받는 생성자(1. 제품의 이름을 나타내는 문자열, 2.
          제품의 가격을 나타내는 문자열)를 작성하세요 (
          <ClassContentsCode>float</ClassContentsCode> 또는{" "}
          <ClassContentsCode>double</ClassContentsCode>
          사용 금지)
          <br />- <ClassContentsCode>Product</ClassContentsCode> 객체가 수행해야
          할 아래의 메서드를 작성하세요.
          <ClassContentsTextTab>
            · <ClassContentsCode>getName()</ClassContentsCode> : 제품의 이름을
            반환하는 메서드
            <br />· <ClassContentsCode>getPrice()</ClassContentsCode> : 제품의
            가격을 BigDecimal로 반환하는 메서드 (
            <ClassContentsCode>float</ClassContentsCode>나{" "}
            <ClassContentsCode>double</ClassContentsCode>을 사용하지 마세요)
          </ClassContentsTextTab>
          - <ClassContentsCode>Drive</ClassContentsCode>의{" "}
          <ClassContentsCode>main()</ClassContentsCode> 메서드에서 다양한{" "}
          <ClassContentsCode>Product</ClassContentsCode> 객체를 생성하고,
          메서드를 호출하여 코드가 제대로 작동하는지 테스트하세요.
          <br />
          <br />
          <b>2. Order 클래스 생성</b>
          <br />- <ClassContentsCode>Order.java</ClassContentsCode> 파일을
          프로젝트 폴더에 추가하고 <ClassContentsCode>Order</ClassContentsCode>{" "}
          클래스를 정의하세요.
          <br />- 모든 <ClassContentsCode>Order</ClassContentsCode> 객체가 가질
          멤버 변수를 정의하세요.
          <ClassContentsTextTab>
            · <ClassContentsCode>products</ClassContentsCode>:{" "}
            <ClassContentsCode>Product</ClassContentsCode>
            객체를 저장하는 <ClassContentsCode>ArrayList</ClassContentsCode>
            <br />· <ClassContentsCode>total</ClassContentsCode>: 각 제품 가격의
            합계를 <ClassContentsCode>BigDecimal</ClassContentsCode>로 저장하는
            변수 (<ClassContentsCode>float</ClassContentsCode>나{" "}
            <ClassContentsCode>double</ClassContentsCode>을 사용하면 과제에서
            0점을 받습니다)
          </ClassContentsTextTab>
          - 입력값이 없는 생성자를 작성하여{" "}
          ...
          초기화하세요.
          <br />- <ClassContentsCode>Order</ClassContentsCode> 객체가 수행해야
          할 아래의 메서드를 작성하세요.
          <ClassContentsTextTab>
            · <ClassContentsCode>addItem</ClassContentsCode> :
            <ClassContentsCode>Product</ClassContentsCode>를 입력으로 받아{" "}
            <ClassContentsCode>ArrayList</ClassContentsCode>와
            <ClassContentsCode>total</ClassContentsCode>을 업데이트하는 메서드
            <br />· <ClassContentsCode>getItem</ClassContentsCode> : 사용자가
            제품 이름을 입력하면 <ClassContentsCode>Order</ClassContentsCode>{" "}
            내에서 해당 이름과 일치하는{" "}
            <ClassContentsCode>Product</ClassContentsCode>를 찾아 반환하는
            메서드. 제품을 찾으면 해당 제품을 반환하고, 찾지 못하면{" "}
            <ClassContentsCode>null</ClassContentsCode>을 반환합니다. 여러
            제품이 같은 이름을 가질 경우 첫 번째로 찾은 제품을 반환합니다.
            <br />· <b>중요</b> : 문자열 비교에{" "}
            <ClassContentsCode>==</ClassContentsCode> 연산자를 사용하지 마세요.
            대신 Java API를 탐색하여 두 문자열이 같은지 비교하는 적절한 메서드를
            찾아 사용하세요.
            <br />· <ClassContentsCode>removeItem</ClassContentsCode> : 제품
            이름을 입력받아 <ClassContentsCode>ArrayList</ClassContentsCode>에서
            해당 이름의 제품을 제거하고,{" "}
            <ClassContentsCode>total</ClassContentsCode>을 업데이트하는 메서드.
            이름이 일치하는 제품을 찾으면 제거 후
            <ClassContentsCode>true</ClassContentsCode>를 반환하고, 찾지 못하면{" "}
            <ClassContentsCode>false</ClassContentsCode>를 반환합니다. 여러
            제품이 같은 이름을 가질 경우 첫 번째로 찾은 제품을 제거합니다.
            <br />· <ClassContentsCode>calculateFinalPrice</ClassContentsCode> :
            판매세를 저장하는 <ClassContentsCode>BigDecimal</ClassContentsCode>
            을 입력으로 받아 (<ClassContentsCode>예시: 0.06</ClassContentsCode>
            ), 판매세가 적용된 <ClassContentsCode>Order</ClassContentsCode>의
            최종 가격을 계산하는 메서드입니다. 이 메서드는 계산이 끝나면
            <ClassContentsCode>BigDecimal</ClassContentsCode>을 반환합니다 (
            <ClassContentsCode>float</ClassContentsCode>나{" "}
            <ClassContentsCode>double</ClassContentsCode> 사용 금지)
            <br />· <b>중요</b> : 판매세 적용 후 소수점 이하 자릿수가 발생할 수
            있습니다 (예시: 총액이 4.22달러이고 판매세가 6%일 경우 최종 가격은
            4.4732달러가 됩니다). 따라서 가장 가까운 센트로 내림하여
            반올림하세요 (예시: 4.4732는 4.47로 반올림).{" "}
            <ClassContentsCode>BigDecimal</ClassContentsCode> 값을 반올림하는
            방법에 대한 간단한 튜토리얼을 참조하세요.
          </ClassContentsTextTab>
          <br />
          <br />
          <b>3. Driver 구현</b>
          ...
          <ClassContentsCode>main()</ClassContentsCode> 메서드를 사용하여{" "}
          <ClassContentsCode>Order</ClassContentsCode>객체를 생성하고 다양한
          값의 <ClassContentsCode>Product</ClassContentsCode> 객체를 추가하여{" "}
          <ClassContentsCode>Order</ClassContentsCode> 클래스의 모든 메서드가
          제대로 작동하는지 테스트하세요.
          <Java_06_03_Code21 />
        </ClassContentsText>
        <br />
