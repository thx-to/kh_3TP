...
import Java_07_01_M from "./Java_07_01_M";
...
public class Sample implements Runnable {
    ...
    public static void main(String[] args) {
        Runnable task = new Sample(); //Runnable은 작업 내용을 가지고 있는 객체이지만 실제 스레드가 아님
        ...
        <Wrap>
          ...
          <Container>
            ...
            <RightContainer>
              <EachClass>
                <ClassHeader>
                  <ClassHeaderTitle>Thread</ClassHeaderTitle>
                </ClassHeader>
                <ClassContentsContainer>
                  <ClassContentsTitle1>
                  ...
                  <ClassContentsText>
                    멀티 태스킹이란 두 가지 이상의 작업을 동시에 처리하는 것을
                    말합니다. 예를 들어{" "}
                    <b>워드로 문서작업을 하는 동시에 음악을 듣는 것</b>은 OS가
                    프로세스마다 작업을 병렬로 처리하기에 가능합니다. 멀티
                    태스킹이 꼭 멀티 프로세스(워드 + 윈도 플레이어 프로세스
                    조합)를 말하는 것은 아닙니다. 
                    <b>
                      한 프로세스 내에서 멀티 태스킹을 할 수 있도록 만들어진
                      프로세스
                    </b>
                    , 예를 들어 메신져 프로세스 같은 경우 채팅 기능을 제공하면서
                    동시에 파일 업로드 기능을 수행할 수 있습니다. 이처럼 한
                    프로세스에서 멀티 태스킹이 가능한 이유는 
                    <b>멀티 스레드(Multi Thread)</b> 덕분입니다. 멀티 프로세스는
                    프로세스마다 운영체제로부터 할당받은 고유의 메모리를 서로
                    침범할 수 없지만 멀티 스레드는 /java/java-jvm/ 포트스에서
                    확인할 수 있는 것처럼 프로세스 내부에서의 멀티 스레드는
                    공유되는 자원이 있어 하나의 스레드에서 예외가 발생한다면
                    프로세스 자체가 종료될 수 있습니다.
                  </ClassContentsText>
                  <br />
                  ...
                  <ClassContentsText>
                    <b>
                      모든 자바 어플리케이션은 Main Thread가 main() 메소드를
                      실행하면서 시작됩니다.
                    </b>
                    <br />
                    예외는 없습니다. 이러한 Main Thread 흐름 안에서 싱글
                    스레드가 아닌 멀티 스레드 어플리케이션은 필요에 따라 작업
                    스레드를 만들어 병렬로 코드를 실행할 수 있습니다. 싱글
                    스레드 같은 경우 메인 스레드가 종료되면 프로세스도
                    종료되지만, 멀티 스레드는 
                    <b>
                      메인 스레드가 종료되더라도 실행 중인 스레드가 하나라도
                      있다면 프로세스는 종료되지 않습니다.
                    </b>
                  </ClassContentsText>
                  <br />
                  ...
                  <ClassContentsText>
                    start() 메소드를 호출하면 작업 스레드는 자신의 run()
                    메소드를 실행 합니다.
                    <Java_07_01_Code01 />
                  </ClassContentsText>
                  <br />
                  ...
                  <ClassContentsText>
                    ...
                    다른 스레드에서 할 작업을 할당하는 방법 입니다.
                    <br />
                    Thread 객체를 생성할 때는 Runnable 인터페이스를 구현한
                    클래스 객체를 매개변수로 받습니다.
                    <br />
                    start() 메서드를 호출하면 작업 스레드는 자신의 run()
                    메소드를 실행 합니다.
                    <Java_07_01_Code02 />
                  </ClassContentsText>
                  <br />
                  ...
                  <ClassContentsText>
                    ...
                    )으로 실행됩니다. 동시성이랑 멀티작업을 위해 1개의 코어로
                    스레드마다 돌아가면서 조금씩 실행하지만, 너무 빨라 사람의
                    눈에 보기에는 독립적으로 돌아가는 것처럼 보이는 것을 말하며
                    병렬성이란 스레드마다 각각의 독립적인 Core가 할당되어
                    독립적인 Core에서 작업이 이루어지는 것을 말합니다. 코어의
                    수보다 스레드의 수가 작으면 각각의 코어로 병렬성이
                    보장되지만 스레드의 개수가 코어보다 많은 경우 스레드를 어떤
                    순서에 의해 동시적으로 실행할 것인가를 결정해 주어야 합니다.
                    이것을 스레드 스케줄링이라 합니다. 이런 스케쥴링 방식은
                  ...
                  <ClassContentsText>
                    시간 할당량을 정해서 하나의 스레드를 정해진 시간만큼만
                    실행하는 방법입니다. 해당 방식은 JVM안에서 이루어지기 때문에
                    개발자가 제어할 수 없습니다.
                  </ClassContentsText>
                  <ClassContentsTitle1>스레드 상태</ClassContentsTitle1>
                  <ClassContentsText>
                    스레드로 객체를 생성하면 우선 실행 대기 상태로 들어갑니다. 
                    <b>실행 대기</b> 상태란 아직 스케쥴링 전 즉 코어에서 작업을
                    할당받지 못한 상태입니다. 실행 대기 상태에 있는 스레드 중
                    스레드 스케쥴링으로 선택된 스레드가 CPU를 점유하고 run()을
                    실행합니다. 이때 CPU를 점유하고 있는 상태가 <b>실행</b>{" "}
                    상태입니다. 아직 run()메소드가 끝나지 않아도 스레드
                    스케쥴링에 의해 다시 실행 대기 상태로 돌아가며 번갈아 가면서
                    내용을 끝낼 때까지 지속합니다. 경우에 따라 run중인 스레드를 
                    <b>일시 정지</b> 시킬 수도 있는데 예제에서 많이 보았던
                    sleep() 메서드와 같은 경우입니다. 일시 정지 후에는 다시 실행
                    대기 상태로 갑니다.
                  </ClassContentsText>
                  <br />
                  <ClassContentsTitle2>스레드 상태 제어</ClassContentsTitle2>
                  <ClassContentsText>
                    실행 중인 스레드 상태를 변경하는 것을 스레드 상태 제어라고
                    합니다. 멀티 프로그램을 만들기 위해서는 정교한 스레드 상태
                    제어가 필요합니다. 상태를 제어하는 메소드는 아래 그림과
                    같습니다.
                    <ClassContentsImage
                      style={{
                        width: "100%",
                    ...
                    <ClassTableBox>
                      <ClassTable style={{ textAlign: "center" }}>
                        ...
                        <ClassTableTr>
                          ...
                          <ClassTableTd>
                            일시 정지 상태의 스레드에서 InterruptException
                            예외를 발생시켜, 예외 처리 코드{" "}
                          </ClassTableTd>
                        </ClassTableTr>
                        <ClassTableTr>
                          ...
                          <ClassTableTd>
                            동기화 블록 내에서 wait() 메소드에 의해 일시 정지
                            상태에 있는 스레드를 실행 대기 상태로 만듦
                          </ClassTableTd>
                        </ClassTableTr>
                        <ClassTableTr>
                          <ClassTableTd>
                          ...
                          <ClassTableTd>
                            suspend() 메소드에 의해 일시 정지 상태에 있는
                            스레드를 대기 상태로 만든다.(Deprecated 됨, 대신에
                            notify(), notifyAll() 사용)
                          </ClassTableTd>
                        </ClassTableTr>
                        <ClassTableTr>
                        ...
                        <ClassTableTr>
                          ...
                          <ClassTableTd>
                            join()메소드를 호출한 스레드는 일시 정지 상태가
                            된다. 실행 대기 상태로 가려면 join() 메소드를 멤버로
                            가진 스레드가 종료 되거나, 매개값으로 주어진 시간이
                            지나야 한다.
                          </ClassTableTd>
                        </ClassTableTr>
                        <ClassTableTr>
                          ...
                          <ClassTableTd>
                            ...
                            매개변수가 주어지면 주어진 시간이 지나면 자동으로
                            실행 대기 상태가 된다. 시간이 주어지진 않으면
                            notify(), notifyAll() 메소드에 의해 실행 대기 상태로
                            전환
                          </ClassTableTd>
                        </ClassTableTr>
                        <ClassTableTr>
                          ...
                          <ClassTableTd>
                            스레드를 일시 정지 상태로 만든다. (Deprecated 됨,
                            대신 wait() 사용)
                          </ClassTableTd>
                        </ClassTableTr>
                        <ClassTableTr>
                  ...
                  <ClassContentsText>
                    실행 중인 스레드를 일정 시간 멈추고 싶다면 Thread 클래스의
                    정적 매소드인 sleep()메소드를 사용하면 됩니다.
                    <br />
                    Thread.sleep(시간) 메소드를 호출한 스레드는 주어진 시간 동안
                    일시 정지 상태가 되고, 이후 다시 실행 상태로 돌아갑니다.
                    <Java_07_01_Code06 />
                  </ClassContentsText>
                  <ClassContentsTitle3>
                    3초 주기로 10번 비프음 발생
                  ...
                  <ClassContentsText>
                    스레드가 처리하는 작업은 반복적인 실행을 위해 for이나
                    while문을 포함하는 경우가 많습니다.
                    <br />
                    무의미한 반복 실행 경우에 다른 스레드에게 실행을 양보하는
                    경우에 사용하는 메소드 입니다.
                    <Java_07_01_Code08 />
                    <Java_07_01_Code09 />
                    <Java_07_01_Code10 />
                  ...
                  <ClassContentsText>
                    ...
                    가 있습니다. 예를 들어 계산 작업을 하는 스레드가 모든 계산을
                    마치고 결과값을 받아 이용해야 하는 경우가 이에 해당 합니다.
                    <br />
                  ...
                  <ClassContentsText>
                    - 경우에 따라 두 개의 스레드가 번갈아가며 실행해야 할 경우가
                    있습니다.
                    <br />- 한 스레드가 작업을 완료하면{" "}
                    <b>
                      <font color="red">notify() 메소드</font>를 호출
                    </b>
                    해서 일시 정지 상태에 있는 다른 스레드를 실행 대기 상태로
                    만들고, 자신은 두번째 작업을 하지 않도록{" "}
                    <b>
                    ...
                    는 wait()에 의해 일시 정지된 모든 스레드를 실행 대기 상태로
                    만듭니다.
                  </ClassContentsText>
                  <ClassContentsTitle3>
                  ...
                  <ClassContentsText>
                    한번에 한개의 스레드만 프로세스 공유 자원에 접근 할 수
                    있도록 락(Lock)을 걸어 다른 스레드가 진행 중인 작업을
                    간섭하지 못하도록 하는 것
                    <ClassContentsImage
                      style={{
                        width: "100%",
                  ...
                  <ClassContentsText>
                    아래의 코드에서 Thread객체를 생성하는 방법은 익명의 객체를
                    람다식으로 구현했습니다.
                    <br />
                    익명의 객체가 아닌 경우에는 Thread클래스를 상속 받은 클래스
                    내부에서 기능을 구현하지만 익명의 객체는 {} 내에서 바로
                    기능을 구현하는 방식 입니다.
                    <Java_07_01_Code22 />
                  </ClassContentsText>
                  <br />
                  ...
                  <ClassContentsText>
                    ...
                    블록을 사용하면 됩니다.`}
                    <Java_07_01_Code23 />
                    하지만 Synchronized 키워드를 너무 남발하면 오히려 프로그램
                    성능저하를 일으킬 수 있기 때문에 적재 적소에 잘 사용해야
                    합니다.
                  </ClassContentsText>
                  <br />
                  ...
                  <ClassContentsText>
                    ...
                    입니다.
                    <br />- 데몬 스레드 이외의 스레드가 모두 종료 되면 데몬
                    스레드도 강제 종료 됩니다.
                    <br />- start() 메소드 호출 전에 setDeamon(true)를 호출해야
                    합니다.
                    <Java_07_01_Code24 />
                  </ClassContentsText>
                  <ClassContentsTitle3>
                  ...
                  <ClassContentsText>
                    기본 스레드는 thread.getName()을 통해 Thread-n이라는 이름을
                    가지고 올 수 있습니다. 하지만 조금 더 수월한 디버깅을 위해
                    thread.setName(“스레드 이름”)을 통해 스레드의 이름을 설정할
                    수 있습니다.
                    <Java_07_01_Code27 />
                  </ClassContentsText>
                  <br />
                  ...
                  <ClassContentsText>
                    인터럽트란? 입출력 장치에서 예외사항이 발생하여 처리가
                    필요할 경우 프로세서에데 알려 처리 할 수 있도록 하는 것
                  </ClassContentsText>
                  <ClassContentsTitle3>
                  ...
                  <ClassContentsText>
                    ...
                    Thread.sleep()이나 Object.wait()메소드와 같은 블로킹
                    메소드는 인터럽트 상태를 확인하고 있다가 인터럽트가 걸리면
                    즉시 리턴된다. 위 두 메서드는 대기하던 중에 인터럽트가
                    걸리면 인터럽트 상태를 해제하면서 InterruptedException을
                    던진다. 여기서 던지는 InterruptedException은 인터럽트가
                    발생해 대기 중이던 상태가 예상보다 빨리 끝났다는 것을
                    뜻한다.
                  </ClassContentsText>
                </ClassContentsContainer>
              </EachClass>
