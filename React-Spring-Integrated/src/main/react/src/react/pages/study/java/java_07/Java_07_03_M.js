  ...
  return (
    <Wrap>
      ...
      <Container>
        ...
        <ClassContentsText>
          프로그램에서 오류가 발생하는 상황은 크게 두 가지로 나누어 집니다.
          하나는 <b>프로그램 코드 작성 중 실수로 발생하는 컴파일 오류</b>
          이고, 다른 하나는{" "}
          <b>
            실행중인 프로그램이 의도하지 않은 동작을 하거나 프로그램이 중지되는
            실행 오류
          </b>
          입니다. 이 중 실행 오류는 프로그램 실행 중에 발생하는 오류는 예측하기
          어렵고, 프로그램이 비정상 상황이 되어 갑자기 멈추는 문제가 발생 할 수
          있습니다.
          <br />
          자바는 이러한 비정상 종료를 줄이기 위해 다양한 예외처리 방법을
          제공하고 있습니다.
          <br />
          예외처리의 목적은 프로그램이 비정상 종료되는 것을 막기 위함 입니다.
        </ClassContentsText>
        <br />
        ...
        <ClassContentsText>
          ...
          <b>
            <font color="red">시스템 오류(Error)</font>이고 다른 하나는{" "}
            <font color="red">예외(Exception)</font>
          </b>
          입니다.
          <br />
          <b>시스템 오류는 제어 할 수 없는 반면에 예외는 프로그램에서 제어</b>
          할 수 있습니다.
          <br />
          예외의 프로그램에서 파일을 읽을 수 없는 경우, 네트워크로 데이터를 주고
          받고자 하는데 네트워크가 연결이 안된 경우, 배열 값을 출력하고자하는데
          배열 요소가 없는 경우 등이 해당 됩니다.
          <br />
        ...
        <ClassContentsText>
          컴파일러가 체크하는 예외로, 코드 작성 시 반드시 예외 처리 코드가
          작성되어야 합니다. 주로 입출력(IO) 관련 예외나 데이터베이스 연결 예외
          등이 여기에 해당됩니다. Checked 예외를 처리하기 위해{" "}
          <ClassContentsCode>try-catch</ClassContentsCode> 블록이나
          <ClassContentsCode>throws</ClassContentsCode> 절을 사용합니다.
        </ClassContentsText>
        <ClassContentsTitle3>Unchecked 예외</ClassContentsTitle3>
        <ClassContentsText>
          <b>
            컴파일러가 체크하지 않는 예외로, 주로 런타임 에러에 해당됩니다. 배열
            인덱스 초과, 널 포인터 참조 등이 여기에 해당
          </b>
          합니다. 이러한 예외는 명시적인 예외 처리가 필요하지 않습니다. 하지만
          프로그램이 정상적으로 실행되지 않을 수 있으므로 이를 피하기 위해
          적절한 예외 처리 또는 예외 발생을 방지하는 로직을 작성하는 것이
          중요합니다.
        </ClassContentsText>
        <br />
        <ClassContentsTitle2>예외 클래스의 종류</ClassContentsTitle2>
        <ClassContentsText>
          프로그램에서 처리하는 예외 클래스의 최상위 클래스는 Exception
          클래스입니다.
          <ClassContentsImage
            style={{
              width: "100%",
        ...
        <ClassContentsText>
          -{" "}
          <b>자바는 어떻게 할 수 없는 에러와 대응 가능한 예외라는 것이 존재</b>
          합니다.
          <br />- 프로그램에서 예외 상황이 발생 했을 경우 프로그램의 갑작스러운
          종료를 막고, 정상적인 실행을 유지 할 수 있도록 처리하는 코드를
          예외처리 코드라고 합니다.
          <br />-{" "}
          <b>
            자바 컴파일러는 소스 파일을 컴파일 할 때 일반 예외가 발생할 가능성이
            있는 코드를 발견하면 컴파일 오류를 발생시켜 개발자로 하여금
            강제적으로 예외 처리를 하도록 요구
          </b>
          합니다.
          <br />- 하지만 <b>실행 시 발생하는 예외는 개발자가 처리</b> 해야
          합니다.
          <br />- 예외처리 코드는 <b>try ~ catch ~ finally</b> 블록을 이용
          합니다.
        </ClassContentsText>
        ...
        <Java_07_03_Code02 />
        <ClassContentsTitle3>FileNotFoundException</ClassContentsTitle3>
        <ClassContentsText>
          존재하지 않는 파일을 열려고 시도 할 때 발생 합니다.
          <Java_07_03_Code03 />
          <Java_07_03_Code04 />
        </ClassContentsText>
        <ClassContentsTitle3>NullPointerException</ClassContentsTitle3>
        <ClassContentsText>
          Java의 NullPointerException은 런타임 예외입니다. Java는 개체 참조에
          특별한 null 값을 할당합니다. 프로그램이 null 값으로 설정된 개체 참조를
          사용하려고하면이 예외가 throw됩니다.
          <br />
          null값은 일반적으로 참조 변수에 값이 지정되지 안았음을 의미 합니다.
          <Java_07_03_Code05 />
        </ClassContentsText>
        ...
        <ClassContentsText>
          ...
          <br />
          - 정의 : 프로그램 실행 시 발생할 수 있는 예외의 발생에 대비한 코드를
          작성하는 것<br />
          - 목적 : 프로그램의 비정상 종료를 막고, 정상적인 실행 상태를 유지하는
          것
          <Java_07_03_Code07 />
        </ClassContentsText>
        <ClassContentsTitle3>finally</ClassContentsTitle3>
        <ClassContentsText>
          프로그램 수행 도중 예외가 발생하면 프로그램이 중지되거나 예외 처리에
          의해 catch 구문이 실행된다. 하지만 어떤 예외가 발생하더라도 반드시
          실행되어야 하는 부분이 있어야 한다면 어떻게 해야 할까?
          <Java_07_03_Code08 />
        </ClassContentsText>
        <ClassContentsTitle3>파일 입출력 예외처리</ClassContentsTitle3>
        <Java_07_03_Code09 />
        <br />
        <ClassContentsTitle2>예외 처리 미루기</ClassContentsTitle2>
        <ClassContentsText>
          해당 메소드 안에서 발생한 예외 처리에 대한 책임을 메소드를 호출한
          쪽으로 떠넘길 수 있습니다. 컴파일 시에는 문제가 발생하지 않았지만 동작
          중 예외 발생 시 호출하는 코드에서 예외를 처리하지 못 한다면 프로그램에
          비정상 종료를 하게 됩니다.
          <Java_07_03_Code10 />
          <Java_07_03_Code11 />
        </ClassContentsText>
        ...
        <ClassContentsText>
          원하는 조건 상황에서 필요한 경우 예외 객체를 생성시키면서 예외를 발생
          시킬 수 있습니다.
          <br />
          자바에서 제공되는 예외 클래스이든 개발자가 만든 사용자 정의 예외
          클래스 이든 모두 생성하고 발생 시킬 수 있습니다.
          <Java_07_03_Code12 />
          <Java_07_03_Code13 />
        </ClassContentsText>
        <br />
        ...
        <ClassContentsText>
          자바에서는 제공해 주는 예외 클래스가 사용하려는 목적에 맞지 않을 때
          직접 예외 클래스를 만들어 사용 할 수 있습니다. 사용자 정의 예외
          클래스를 만들 때 java.lang.Exception 클래스를 상속 받아야 합니다.
          <Java_07_03_Code14 />
          <Java_07_03_Code15 />
          <Java_07_03_Code16 />
        </ClassContentsText>
