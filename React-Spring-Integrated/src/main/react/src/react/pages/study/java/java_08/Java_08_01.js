...
import Java_08_01_M from "./Java_08_01_M";
        ...
        <Wrap>
          ...
          <Container>
            ...
            <RightContainer>
              <EachClass>
                <ClassHeader>
                  <ClassHeaderTitle>함수형 프로그래밍</ClassHeaderTitle>
                </ClassHeader>
                <ClassContentsContainer>
                  <ClassContentsTitle2>
                    자바에서의 함수형프로그래밍
                  ...
                  <ClassContentsText>
                    - 자바는 <b>Java 8 버전</b>부터 함수형 프로그래밍을 지원하기
                    위해 <b>람다(Lambda)와 스트림(Stream)</b>이 도입
                    <br />-{" "}
                    <b>
                      람다와 스트림을 사용하면 요즘 유행하는 함수형 프로그래밍
                      스타일로 자바 코드를 작성
                    </b>
                    <br />
                    <br />
                    물론 람다와 스트림을 사용하여 작성한 코드를 일반 스타일의
                    자바코드로 바꾸어 작성하는 것이 불가능하지는 않다. 달리
                    말하면 람다와 스트림 없이도 자바 코드를 작성하는데 어려움이
                    없다는 뜻이다. 하지만 그럼에도 불구하고 람다와 스트림을
                    사용하는 이유는 작성하는 코드의 양이 줄어들고 읽기 쉬운
                    코드를 만드는데 유리하기 때문이다.
                  </ClassContentsText>
                  <br />
                  <ClassContentsTitle2>
                  ...
                  <ClassContentsText>
                    - 함수형 프로그래밍은 하나의 프로그래밍 패러다임으로
                    정의되는 일련의 코딩 접근 방식이며, 
                    <b>
                      <u>
                        자료처리를 수학적 함수의 계산으로 취급하고 상태와 가변
                        데이터를 멀리하는 프로그래밍 패러다임
                      </u>
                    </b>
                    을 의미
                    <br />
                  ...
                  <ClassContentsText>
                    1. <b>불변성(Immutability)</b> : 함수형 프로그래밍에서
                    데이터는 변경할 수 없는(immutable) 것으로 취급됩니다.
                    데이터를 변경하고 싶을 때는, 변경 사항을 적용한 새로운
                    데이터 복사본을 생성합니다. 이는 프로그램의 상태를 더 쉽게
                    추적할 수 있게 해줍니다.
                    <br />
                    2.{" "}
                    <b>
                      함수의 일급 객체(First-Class and Higher-Order Functions)
                    </b>{" "}
                    : 함수는 "일급 객체"로 취급됩니다. 이는 함수를 변수에 할당할
                    수 있으며, 다른 함수의 인자로 전달하거나,{" "}
                    <b>함수로부터 반환값을 받을 수 있음을 의미</b>합니다. 더
                    나아가, 함수는 다른 함수를 생성하고 반환할 수 있는 "고차
                    함수(Higher-Order Functions)"로 작용할 수 있습니다.
                    <br />
                    3. <b>순수 함수(Pure Functions)</b> : 순수 함수는 동일한
                    인자에 대해 항상 동일한 결과를 반환하며, 외부 상태를
                    변경하지 않고, 외부 상태에 의존하지 않는 함수를 말합니다.
                    순수 함수의 사용은 프로그램의 사이드 이펙트를 줄이고, 코드의
                    테스트와 재사용을 용이하게 합니다.
                    <br />
                    4. <b>함수 조합(Function Composition)</b> : 함수 조합은{" "}
                    <b>여러 함수를 하나의 연산으로 결합</b>하는 것을 의미합니다.
                    이를 통해 복잡한 처리를 간결하고 명확한 코드로 표현할 수
                    있습니다.
                    <br />
                    5. <b>재귀(Recursion)</b> : 함수형 프로그래밍에서는 반복문
                    대신 재귀를 사용하여 반복적인 처리를 구현합니다. 재귀는
                    함수가 자기 자신을 호출하는 것을 의미하며, 이를 통해 코드의
                    가독성을 높일 수 있습니다.
                  </ClassContentsText>
                  <br />
                  <ClassContentsTitle1>람다(Lambda)</ClassContentsTitle1>
                  <ClassContentsText>
                    자바 8부터 도입된 기능으로, 간단한 익명 함수를 생성하기 위한
                    방법을 제공합니다.{" "}
                    <b>
                      람다 표현식은 주로 함수형 프로그래밍 스타일을 지원하며,
                      컬렉션 처리나 멀티스레딩과 같은 상황에서 유용하게 활용
                    </b>
                    됩니다.
                  </ClassContentsText>
                  <br />
                  ...
                  <ClassContentsText>
                    ...
                    - 매개변수의 타입을 추론할 수 있는 경우에는 타입을 생략 할
                    수 있습니다.
                    <br />- 매개변수가 하나인 경우에는 괄호()를 생략 할 수
                    있습니다.
                    <br />- 함수의 몸체가 하나의 명령문만으로 이루어진 경우에는
                    중괄호{}를 생략 할 수 있습니다.
                    <Java_08_01_Code01 />
                  </ClassContentsText>
                  <br />
                  ...
                  <ClassContentsText>
                    중괄호 안의 구현 부분이 한 문장인 경우 중괄호 생략 할 수
                    있습니다.
                    <Java_08_01_Code03 />
                  </ClassContentsText>
                  ...
                  <ClassContentsText>
                    -{" "}
                    <b>
                      람다식은 메소드 이름이 없고 메소드를 실행하는데 필요한
                      매개변수와 매개변수를 활용한 실행 코드를 구현하는 것
                    </b>
                    <br />- 함수형 언어에서는 함수만 따로 호출 할 수 있지만,
                    자바는 참조 변수없이 메소드를 호출 할 수 없습니다. 그러므로
                    <b>
                    ...
                    <br />
                    위의 문법처럼 람다표현식을 하나의 변수에 대입할 때 사용하는
                    참조 변수의 타입을 함수형 인터페이스 라고 부릅니다.
                    <br />
                    함수형 인터페이스는 추상 클래스와 달리{" "}
                    <b>단 하나의 추상 메소드</b>만을 가져야 합니다.
                    <br />
                    또한, 다음과 같은{" "}
                    <b>어노테이션을 사용하여 함수형 인터페이스임을 명시</b>할 수
                    있습니다.
                    <br />
                    <ClassContentsCode>@FunctionalInterface</ClassContentsCode>
                    <br />
                    위와 같은 어노테이션을 인터페이스의 선언 앞에 붙이면,
                    컴파일러는 해당 인터페이스를 함수형 인터페이스로 인식
                    합니다. 자바 컴파일러는 이렇게{" "}
                    <b>
                      명시된 함수형 인터페이스에 두 개 이상의 메소드가 선언되면
                      오류를 발생
                    </b>
                    시킵니다.
                  </ClassContentsText>
                  ...
                  <ClassContentsText>
                    - 메서드 참조(method reference)는 메소드를 참조해서 매개
                    변수의 정보 및 리턴 타입을 알아 내어, 불 필요한 매개 변수를
                    제거하는 것이 목적
                    <br />- 메서드 참조는 기본적으로 메서드 이름을 직접
                    참조하여, 마치 람다 표현식처럼 사용할 수 있는 기능
                  </ClassContentsText>
                  <ClassContentsTitle3>
                    정적 메소드와 인스턴스 메소드 참조
                  </ClassContentsTitle3>
                  <ClassContentsText>
                    ...
                    인터페이스에서 입력값을 받아 int 결과를 반환하는 메서드
                    (기본형 특화 함수형인터페이스)
                  </ClassContentsText>
                  <ClassContentsTitle3>
                    매개 변수의 메소드 참조
                  ...
                  <ClassContentsText>
                    ...
                    : 두 문자열을 대소문자와 상관없이 동일한 알파벳으로 구성되어
                    있는지 비교
                    <Java_08_01_Code11 />
                  </ClassContentsText>
                  ...
                  <ClassContentsText>
                    ...
                    <font color="#2c79c1">
                      <i>
                        <b>
                          DoubleUnaryOperator 인터페이스는 한 개의 double 형
                          매개변수를 전달받아 한 개의 double 형 값을 반환하는
                          java.util.function 패키지에서 제공하는 함수형
                          인터페이스입니다.
                        </b>
                      </i>
                    </font>
