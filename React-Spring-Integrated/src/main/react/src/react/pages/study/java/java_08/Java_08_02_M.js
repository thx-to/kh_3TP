    ...
    <Wrap>
      ...
      <TopBoxWide2>
        <TopBox2>
          ...
          <TopBoxText2 onClick={() => handleRefresh()}>02. 스트림</TopBoxText2>
        </TopBox2>
      </TopBoxWide2>
      <Container>
        {/* <EachClass>
        ...
        <ClassContentsText>
          <b>스트림은 자바 8부터 추가된 기능</b>으로, 배열이나 컬렉션의 데이터를
          일관성 있게 처리할 수 있게 해줍니다. 스트림을 사용하면{" "}
          <b>
            배열, 컬렉션 등의 자료를 반복문 없이 람다식을 이용해 간결하고 일관된
            방식으로 처리
          </b>
          할 수 있습니다. 스트림은 데이터 소스를 변경하지 않고,{" "}
          <b>내부 반복자를 사용해 성능을 최적화하며, 단 한 번만 사용</b>할 수
          있습니다.
        </ClassContentsText>
        <br />
        ...
        <ClassContentsText>
          ...
          수행합니다.
          <br />
          2. <b>내부 반복</b> : 외부 반복이 아닌 내부 반복을 사용하여 작업을
          수행합니다.
          <br />
          3. <b>단일 사용</b> : 스트림은 한 번만 사용할 수 있습니다.
          <br />
          4. <b>원본 데이터 불변</b> : 스트림 연산은 원본 데이터를 변경하지
          않습니다.
          <br />
          5. <b>지연 연산</b> : 중간 연산은 지연(lazy)되어 최종 연산이 호출될 때
          수행됩니다.
          <br />
          6. <b>병렬 처리 지원</b> :{" "}
          <ClassContentsCode>parallelStream()</ClassContentsCode> 메서드를
          사용하여 병렬 처리를 쉽게 수행할 수 있습니다.
        </ClassContentsText>
        <br />
        <ClassContentsTitle2>스트림 API의 동작 흐름</ClassContentsTitle2>
        <ClassContentsText>
          1. <b>스트림 생성</b> :{" "}
          <ClassContentsCode>stream()</ClassContentsCode>,{" "}
          <ClassContentsCode>Arrays.stream()</ClassContentsCode>,{" "}
          <ClassContentsCode>Stream.of()</ClassContentsCode> 등을 사용해
          스트림을 생성합니다.
          <br />
          2. <b>중간 연산</b> : 스트림을 변환하는 연산으로, 필터링, 매핑, 정렬
          등을 수행합니다. (<ClassContentsCode>filter()</ClassContentsCode>,{" "}
          <ClassContentsCode>map()</ClassContentsCode>,{" "}
          <ClassContentsCode>sorted()</ClassContentsCode> 등)
          <br />
          3. <b>종단 연산</b> : 스트림의 요소를 최종 처리하는 연산으로, 요소의
          집계, 반복 등의 작업을 수행합니다. (
        ...
        <ClassContentsTitle3>
          <font color="black">스트림 생성, 중간 연산, 최종 연산 예시</font>
        </ClassContentsTitle3>
        <Java_08_02_Code01 />
        <ClassContentsTitle3>
        ...
        <ClassContentsText>
          ...
          <b>
            멀티 코어 CPU를 최대한 활용하기 위해 요소들을 분배 시켜 병렬 처리
          </b>
          를 할 수 있게 도와준다.
          <Java_08_02_Code04 />
        </ClassContentsText>
        <ClassContentsTitle3>
          <font color="black">스트림을 통한 중간 처리와 최종 처리 예시</font>
        </ClassContentsTitle3>
        <ClassContentsText>
          스트림은 컬렉션의 요소에 대해 중간 처리와 최종 처리를 수행 할 수
          있는데, 중간 처리에서는 매핑, 필터링, 정렬을 수행하고 최종 처리에서는
          반복, 카운팅, 평균, 총합 등의 집계 처리를 수행 합니다.
          <Java_08_02_Code05 />
        </ClassContentsText>
        <br />
        ...
        <ClassContentsText>
          배열에 관한 스트림을 생성하기 위해 Arrays 클래스에는 다양한 형태의
          stream() 메소드가 클래스 메소드로 정의 되어 있습니다.
          <Java_08_02_Code07 />
        </ClassContentsText>
        <ClassContentsTitle3>
        ...
        <ClassContentsText>
          1부터 100까지의 합을 구하기 위해 IntStream의 rangeClosed() 메소드를
          이용
          <Java_08_02_Code08 />
        </ClassContentsText>
        <ClassContentsTitle3>
        ...
        <ClassContentsText>
          Files의 정적 메소드인 lines()와 BufferedReader의 Line() 메소드를
          이용하여 문자 파일의 내용을 스트림을 통해 행 단위로 읽고 콘솔에 출력
          합니다.
          <Java_08_02_Code09 />
        </ClassContentsText>
        <br />
        ...
        <ClassContentsText>
          ...
          데이터의 합계, 평균값, 카운팅, 최대값, 최소값 등이 대표적인 리덕션의
          결과물 입니다. 그러나 컬렉션의 결과물로 바로 집계할 수 없을 경우에는
        ...
        <ClassContentsText>
          스트림 API에 의해 생성된 초기 스트림은 중개 연산을 통해 또 다른
          스트림으로 변환 됩니다.
          <br />
          이러한 중개 연산은 스트림을 전달받아 스트림을 반환하므로, 중개 연산은
          연속으로 연결해서 사용할 수 있습니다.
          <br />
          또한, 스트림의 중개 연산은 필터-맵(filter-map) 기반의 API를 사용함으로
          지연(lazy) 연산을 통해 성능을 최적화할 수 있습니다.
          <br />
          스트림 API에서 사용할 수 있는 대표적인 중개 연산과 그에 따른 메소드는
          다음과 같습니다.
          <br />
          <ClassContentsTextTab>
            1. 스트림 필터링 :{" "}
          ...
          <ClassTableBox>
            <ClassTable style={{ textAlign: "center" }}>
              ...
              <ClassTableTr>
                ...
                <ClassTableTd>
                  각 요소를 <b>여러 </b>값 혹은 <b>스트림</b>으로 변환하고 병합
                </ClassTableTd>
              </ClassTableTr>
              <ClassTableTr>
                <ClassTableTd>
                ...
                <ClassTableTd>평탄화된 하나의 단일 스트림</ClassTableTd>
              </ClassTableTr>
              <ClassTableTr>
                <ClassTableTd>
                  <b>1:1 또는 N:1</b>
        ...
        <ClassContentsText>
          스트림 API에서 중개 연산을 통해 변환된 스트림은 마지막으로 최종 연산을
          통해 각 요소를 소모하여 결과를 표시합니다.
          <br />
          즉, 지연(lazy)되었던 모든 중개 연산들이 최종 연산 시에 모두 수행되는
          것입니다.
          <br />
          이렇게 최종 연산 시에 모든 요소를 소모한 해당 스트림은 더는 사용할 수
          없게 됩니다.
          <br />
          스트림 API에서 사용할 수 있는 대표적인 최종 연산과 그에 따른 메소드는
          다음과 같습니다.
          <br />
          <br />- 요소의 출력 :{" "}
          <font color="red">
        ...
        <ClassContentsText>
          <b>
            스트림에서 제공되는 메소드를 이용해 조건문과 반복문 없이 일관된
            방식으로 처리
          </b>
          할 수 있습니다.
          <Java_08_02_Code19 />
