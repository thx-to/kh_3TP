...
import Java_09_03_M from "./Java_09_03_M";
          ...
          <Container>
            ...
            <RightContainer>
              <EachClass>
                ...
                <ClassContentsContainer>
                  <ClassContentsText>
                    TCP(Transmission Control Protocol)은{" "}
                    <b>연결 지향적 프로토콜</b> 입니다.
                    <br />
                    클라이언트가 연결 요청을 하고, 서버가 연결을 수락하면 통신
                    선로 고정되고, 모든 데이터는 통선 선로를 통해 순차적으로
                    전달 합니다.
                    <br />
                    데이터를 전송하기 전 연결이 형성되어하고 이제 소요되는
                    시간이 많이 걸립니다.
                  </ClassContentsText>
                  <br />
                  <ClassContentsTitle2>
                  ...
                  <ClassContentsText>
                    java.net.Socket 클래스의 역할
                  </ClassContentsText>
                  <ClassContentsTitle3>
                    <font color="black">바이딩 포트</font>
                  </ClassContentsTitle3>
                  <ClassContentsText>
                    서버는 클라이언트가 접속 할 포트를 가지고 있어야 하는데, 이
                    포트를 바인딩 포트라고 합니다.
                  </ClassContentsText>
                  <ClassContentsTitle3>
                  ...
                  <ClassContentsText>
                    ...
                    ServerSocket을 생성할 때 포트번호 하나를 지정해야함
                    <br />
                    서버가 실행되면 클라이언트는 서버의 IP 주소와 바인딩 포트
                    번호로 Socket을 생성하여 연결 요청
                    <br />
                    ServerSocket은 클라이언트가 연결 요청을 하면 accept()
                    메서드로 연결 수락, 통신용 socket 생성
                    <br />그 후 클라이언트와 서버는 각각 Socket을 이용해
                    데이터를 주고 받음
                  </ClassContentsText>
                  <br />
                  <ClassContentsTitle2>
                    ServerSocket 생성과 연결 요청
                  </ClassContentsTitle2>
                  ...
                  <ClassContentsText>
                    ...
                    <Java_09_03_Code01 />
                    <br />
                    두번째 방법 : 디폴트 생성자로 객체 생성 후 포트 바인딩을
                    위해 bind() 메서드 호출
                    <Java_09_03_Code02 />
                    <br />
                    서버PC에 멀티IP가 할당되어 있을 경우, 특정 IP로만 접속할
                    때만 연결 수락을 하고 싶다면 “localhost”대신 정확한 IP를
                    적으면 됩니다.
                    <Java_09_03_Code03 />
                    <br />
                    ...
                    BindException이 발생함
                    <br />
                    {`=>`} 다른 포트로 바인딩하거나 다른 프로그램을 종료하고
                    다시 실행하면 됨.
                  </ClassContentsText>
                  <ClassContentsTitle3>연결 수락</ClassContentsTitle3>
                  <ClassContentsText>
                    포트 바인딩이 끝났으면 ServerSocket은 클라이언트 연결 수락을
                    위해 accept() 메서드 실행하면 됩니다.
                    <br />
                    accept()는 클라이언트가 연결 요청하기 전까지 블로킹
                    <br />
                    블로킹 : 스레드가 대기 상태가 된다는 뜻<br />
                    때문에 UI를 생성하는 스레드나, 이벤트를 처리하는 스레드에서
                    accept()를 호출하지 않아야 함.
                  </ClassContentsText>
                  <ClassContentsTitle3>
                  ...
                  <ClassContentsText>
                    클라이언트가 연결 요청 connect()하고 서버가 연결 수락
                    accept() 했다면, 양쪽의 Socket객체로 부터 각각 입력 스트림과
                    출력 스트림을 얻을 수 있습니다.
                    <ClassContentsImage
                      style={{
                        width: "100%",
                    ...
                    개별 바이트나 문자열인 데이터의 원천 파일을 읽거나 쓸 때,
                    네트워크 소켓을 거쳐 통신 할 때 쓰이는 추상적인 개념을 의미
                    합니다.
                  </ClassContentsText>
                  <ClassContentsTitle3>
                  ...
                  <ClassContentsText>
                    - InputStream은 데이터를 바이트 단위로 읽어 들이는 통로
                    입니다.
                    <br />
                    - InputStream은 추상 클래스이며 입력 스트림을 나타내는 모든
                    클래스의 수퍼 클래스 입니다.
                    <br />
                    - InputStream은 데이터 읽기, 특정 시점으로 되돌아가기, 남아
                    있는 데이터 보여주기 , 끊기 기능 제공 합니다.
                    <Java_09_03_Code09 />
                    <br />
                    <b>Server</b>
                    <Java_09_03_Code10 />
